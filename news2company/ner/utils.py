...
def test_ner(results, path):
    ...
    with open(output_file, "w") as f:
        ...
        f.writelines(to_write)
    eval_lines, counts = return_report(output_file)
    return eval_lines, counts
...
def make_path(params):
    ...
    if not os.path.isdir(params.summary_path):
        os.makedirs(params.summary_path)
...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger=None, is_train=True):
