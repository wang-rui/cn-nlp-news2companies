...
class Model(object):
    def __init__(self, config, is_train=True):
        ...
        with tf.variable_scope('local', reuse=tf.AUTO_REUSE):
            self.global_step = tf.get_variable('Variable',
                                               shape=[],
                                               dtype=tf.int32,
                                               initializer=tf.zeros_initializer(),
                                               trainable=False)
            self.best_dev_f1 = tf.get_variable('Variable_1',
                                               shape=[],
                                               dtype=tf.float32,
                                               initializer=tf.zeros_initializer(),
                                               trainable=False)
            self.best_test_f1 = tf.get_variable('Variable_2',
                                                shape=[],
                                                dtype=tf.float32,
                                                initializer=tf.zeros_initializer(),
                                                trainable=False)
    ...
    def run_step(self, sess, is_train, batch, summary=False):
        """
        ...
        if is_train:
            merged, global_step, loss, _ = sess.run(
                [self.merged, self.global_step, self.loss, self.train_op],
                feed_dict)
            if summary:
                return merged, global_step, loss
            else:
